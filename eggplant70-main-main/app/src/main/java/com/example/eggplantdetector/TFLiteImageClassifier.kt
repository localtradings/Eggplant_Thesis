...
class TFLiteImageClassifier(context: Context) : Closeable {
    ...
    init {
        interpreter = Interpreter(
            loadModelFile(context, "mobile_ar_classifier_cleaned_int8.tflite"),
    ...
    private fun loadModelFile(context: Context, assetName: String): ByteBuffer {
        val modelAssetName = resolveModelAssetName(context, assetName)
    ...
    private fun resolveModelAssetName(context: Context, assetName: String): String {
        return try {
            context.assets.openFd(assetName).close()
            assetName
        } catch (exception: IOException) {
            val fallbackModel = context.assets.list("")
                ?.firstOrNull { it.endsWith(".tflite") }
            if (fallbackModel != null) {
                Log.w(
                    "TFLiteImageClassifier",
                    "Model asset '$assetName' not found; falling back to '$fallbackModel'."
                )
                fallbackModel
            } else {
                throw exception

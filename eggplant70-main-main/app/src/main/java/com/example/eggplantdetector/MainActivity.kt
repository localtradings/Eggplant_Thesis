...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        const val EXTRA_INITIAL_CAPTURE_MODE = "initial_capture_mode"

        fun parseInitialCaptureMode(intent: Intent?): CaptureMode {
            return when (intent?.getStringExtra(EXTRA_INITIAL_CAPTURE_MODE)?.uppercase(Locale.US)) {
                CaptureMode.LIVE.name -> CaptureMode.LIVE
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        captureMode = parseInitialCaptureMode(intent)
        binding = ActivityMainBinding.inflate(layoutInflater)
        setContentView(binding.root)

        cameraExecutor = Executors.newSingleThreadExecutor()
        classifier = try {
            TFLiteImageClassifier(this)
        } catch (error: Throwable) {
            Log.e("EggplantTargeting", "Model failed to load", error)
            Toast.makeText(
                this,
                "Model failed to load. Diagnosis will be unavailable until fixed.",
                Toast.LENGTH_LONG
            ).show()
            null
        }
    ...
    private fun configureUi() {
        binding.modeToggleGroup.check(
            if (captureMode == CaptureMode.LIVE) R.id.liveModeButton else R.id.photoModeButton
        )
        ...
        if (TargetSelectionContract.isPhase1ManualTargetingEnabled) {
            ...
            val initial = if (captureMode == CaptureMode.LIVE) {
    ...
    private fun startCamera() {
        val previewRotation = binding.previewView.display?.rotation ?: 0
    ...
    private fun analyzeImage(imageProxy: ImageProxy) {
        ...
        try {
            // Manual targeting: until the user taps a leaf we only need camera preview pixels for the overlay.
            // Converting every frame to Bitmap would allocate full-resolution buffers every tick and commonly
            // OOM-crash the process as soon as Live mode opens.
            if (TargetSelectionContract.isPhase1ManualTargetingEnabled) {
                when (selectedTargetController.state) {
                    SelectedTargetState.None,
                    is SelectedTargetState.Lost -> return

                    is SelectedTargetState.Selected -> Unit
                }
            }
            ...
            Log.e("EggplantTargeting", "Frame analysis failed", error)
            runOnUiThread {
                if (TargetSelectionContract.isPhase1ManualTargetingEnabled) {
                    selectedTargetController.resetTarget(TargetLossReason.CAMERA_INTERRUPTED)
                    renderTargetState(selectedTargetController.state)
    ...
    private fun buildDebugSnapshot(
        ...
        val previewBitmap = classifier?.prepareDebugBitmap(sourceBitmap) ?: sourceBitmap
        val details = listOf(
            "File: $fileLabel",
